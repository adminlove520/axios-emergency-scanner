...
# Try the exact same format as the original script
$body = '{"userId":"1dd324a2-9b68-4bd3-9eaf-021017fcd2fe"}'
Write-Host "Trying with userId: $body"
try {
...
# Try with the old endpoint too
Write-Host "---"
Write-Host "Trying old endpoint /api/user/follow"
try {
    $response = Invoke-RestMethod -Uri 'https://clawpi.fluxapay.xyz/api/user/follow' -Method POST -ContentType 'application/json' -Headers $headers -Body $body -TimeoutSec 10
    Write-Host "Response: $($response | ConvertTo-Json)"
} catch {
    Write-Host "Error: $($_.Exception.Message)"
    Write-Host "Status Code: $($_.Exception.Response.StatusCode.value__)"
